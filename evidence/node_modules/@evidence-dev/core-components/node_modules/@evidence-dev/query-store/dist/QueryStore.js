...
export class QueryStore extends AbstractStore {
    opts;
    root;
    static activeQueries = writable(new Set());
    ...
    static debug = Boolean(typeof window === 'undefined'
    ...
    #setError = (e) => {
        if (QueryStore.debug)
            console.debug(`QueryStore ${this.id.substring(0, 6)} | QueryStore encountered a non-fatal error`, e instanceof Error ? e?.message ?? e : e);
        this.#error = e;
        if (e) {
    ...
    constructor(query, exec, id, opts = { disableCache: false }, root) {
        ...
        if (typeof query === 'string') {
            ...
            this.#query = query;
        this.#exec = (...args) => exec(args[0], args[1]);
        ...
        if (opts.noResolve && !opts.initialData) {
            ...
            handleMaybePromise((alreadyFetchedMeta) => {
                if (!alreadyFetchedMeta)
                    this.#fetchMetadata();
            }, () => this.#handleInitialData(), this.#setError);
            // prerender
    ...
    backgroundFetch = async () => {
        ...
        handleMaybePromise(() => { }, () => this.#exec(`--data\n${this.#query.toString()}`, this.id), () => { });
    };
    ...
    #fetchData = () => {
        ...
        this.#dataFetchPromise = handleMaybePromise((result) => {
            this.#values = result;
            this.#dataLoading = false;
            this.#dataLoaded = true;
            QueryStore.removeActiveQuery(this.id);
            return this.#fetchLength();
        }, () => this.#exec(queryWithComment, this.id), (err) => {
            this.#setError(err);
            QueryStore.removeActiveQuery(this.id);
        });
        return this.#dataFetchPromise;
    ...
    #fetchLength = () => {
        ...
        return handleMaybePromise((result) => {
            ...
            this.#warnHighScore();
            this.publish();
        }, () => this.#exec(queryWithComment, `${this.id}_length`), this.#setError);
    };
    #fetchMetadata = () => {
        if (this.#error) {
            if (QueryStore.debug)
        ...
        return handleMaybePromise((queryResult) => {
            ...
            this.publish();
        }, () => this.#exec(`--col-metadata\nDESCRIBE ${this.#query.toString()}`, `${this.id}_metadata`), this.#setError);
    };
    #calculateScore = () => {
     ...
     */
    #withStoreCache = (aggKey, aggFunc, passCurrentAsInitial = false) => {
        return (...args) => {
            ...
            const newStore = new QueryStore(newQuery, this.#exec, hash, {
                ...this.opts,
    ...
    groupBy = this.#withStoreCache('groupBy', mutations.groupBy.fn, mutations.groupBy.currentAsInitial);
    agg = this.#withStoreCache('agg', mutations.agg.fn, mutations.agg.currentAsInitial);
    orderBy = this.#withStoreCache('orderBy', mutations.orderBy.fn, mutations.orderBy.currentAsInitial);
